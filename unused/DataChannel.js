...
(function() {
	...
	window.DataChannel.channelExists=function(channel,extras){
		return new Promise(function(success,failure) {
			new window.Firebase('https://' + ((extras && extras.firebase) || 'webrtc-experiment') + '.firebaseIO.com/' + channel).once('value', function(data) {
				console.debug('room is present?', data.val() !== null);
				((data.val() === null)?failure:success)(data.val());
			});
		});
	};
